...
def printNGE(arr):
    ...
    for i in range(1, len(arr), 1):
        next_g = arr[i]
        if not s.isEmpty():
            element = s.pop()  # if stack is not empty, then pop an element from stack
            ...
            while element < next_g:
                print(str(element) + " -- " + str(next_g))
                if s.isEmpty():
                    break
                element = s.pop()

            # If element is greater than next_g, then push the element back
            if element > next_g:
                s.push(element)

        # push next_g to stack so that we can find next_g greater for it
        s.push(next_g)

    # After iterating over the loop, the remaining elements in stack do not have the next_g greater
