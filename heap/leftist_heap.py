...
# Python program for leftist heap / leftist tree


class LeftistNode:
    def __init__(self, element, dist=0, left=None, right=None):
...
class LeftistHeap:
    def __init__(self, root=None, rhs=None):
        self.root = root
        self.rhs = rhs

    def merge_node(self, rhs):
        """
        ...
        if self == rhs:
            return
        self.root = self.merge(self.root, rhs.root)
        rhs.root = None
    ...
    def merge(self, h1, h2):
        ...
        :param h1: LeftistHeap
        :param h2: LeftistHeap
        :return:
        """
        if h1 is None:
            return h2
    ...
    def reclaim_memory(self, t):
        """Internal method to make the tree empty."""
        if t is not None:
            self.reclaim_memory(t.left)
            self.reclaim_memory(t.right)
            del t
    ...
    def clone(self, t):
        ...
        if t is None:
            ...
            return LeftistNode(t.element, self.clone(t.left), self.clone(t.right), t.dist)
    ...
    def find_min(self):
        """
        Find the smallest item in the priority queue. Return the smallest item, or
        throw Underflow if empty"""
        # the smallest item from the priority queue. Throws Underflow if empty.
...
if __name__ == '__main__':
    ...
    min_ptr = Pointer(0)
    h.delete_min(min_ptr)
    print(min_ptr.value)

    h1.delete_min(min_ptr)
    print(min_ptr.value)

    h.merge_node(h1)
    h2 = deepcopy(h)

    h2.delete_min(min_ptr)
    print(min_ptr.value)
