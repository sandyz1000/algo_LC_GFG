...
def can_pairs(arr, n, k):
    ...
    """
    if n & 1:  # An odd length array cannot be divided into pairs
        return False
    ...
    for i in range(n):
        rem = arr[i] % k
        freq[rem] = freq[rem] + 1 if rem in freq else 0
    ...
    for i in range(n):
        ...
        # Else number of occurrences of remainder must be equal to number of occurrences of k -
        # remainder
...
if __name__ == '__main__':
    # Output Pairs: (92, 48), (75, 65) and (45, 35)
    arr = [92, 75, 65, 48, 45, 35]
    k = 10
    n = len(arr)
    print("True" if can_pairs(arr, n, k) else "False")
