...
# Python program to detect cycle in a directed graph
...
class Graph(object):
    ...
    def dfs_util(self, u, color):
        color[u] = Color.GREY

        for v in self.graph[u]:
            if color[v] == Color.GREY:
                return True
            if color[v] == Color.WHITE and self.dfs_util(v, color):
                return True
    ...
    def is_cyclic(self):
        color = [Color.WHITE] * self.V

        for i in range(self.V):
            if color[i] == Color.WHITE:
                if self.dfs_util(i, color):
                    return True
        return False
...
if __name__ == '__main__':
    ...
    g.addEdge(2, 3)
    g.addEdge(3, 3)
    print("Graph contains cycle" if g.is_cyclic() else "Graph doesn't conatin cycle")
